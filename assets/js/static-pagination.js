...
function _staticPagination(page) {
    if (page === 0 && _currentPage !== 1) {
        ...
        for (let i = _currentPage * _perPage - _perPage; i < _currentPage * _perPage; i++) {
            try {
                ...
                $list.append('' +
                    '<li>' +
                    '<div><a href=' + $staticPaginationList[i].created_at + '>' + $staticPaginationList[i].review_title + '</a></div>' +
                    '<div>' + $staticPaginationList[i].body_of_review + '</div>' +
                    '<div><small>' + $staticPaginationList[i].name + '</small></div></li>');
            } catch (exception) {
                break;
            }
        }
    ...
    if (_paginationLength > 1) {
        $pagination.empty();
        if (_currentPage === 1) {
            $pagination.append('<li class="review-page-item disabled">' +
        ...
        for (let _page = 1; _page <= _paginationLength; _page++) {
            if (_page === _currentPage) {
                $pagination.append('<li class="review-page-item active">' +
